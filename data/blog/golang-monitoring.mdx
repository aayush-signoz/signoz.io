...
Golang (Go) applications are known for their high performance, concurrency model, and efficient resource use, making Go an easy choice for building modern distributed systems. Effective monitoring of a Go application means understanding its internals, such as:
...
**Tools to Monitor Golang Applications**
...
Dependencies related to the **[OpenTelemetry exporter](https://signoz.io/guides/opentelemetry-collector-vs-exporter/)** and the SDK must be installed first. 
...
Configure your application to send telemetry data to your observability backend. Start from initializing traces, metrics, and logs in `telemetry/otel.go`.
...
<KeyPointCallout title="Function Breakdown" defaultCollapsed={true}>
...
The above function wires OpenTelemetry logging into the application. It configures an OTLP log exporter, creates a LoggerProvider that batches logs, and attaches a Logrus hook so log statements automatically include trace and span metadata.
...
For sending data to SigNoz cloud, you will need details like [ingestion key](https://signoz.io/docs/ingestion/signoz-cloud/keys/) and [region](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint). You can find them under `Workspace Settings` in `Settings`.

Run the following command after fetching the above details:

```bash
SERVICE_NAME=goApp INSECURE_MODE=false REDIS_ADDR=<REDIS-ADDRESS>  OTEL_EXPORTER_OTLP_HEADERS="signoz-ingestion-key=<SIGNOZ-INGESTION-KEY>" OTEL_EXPORTER_OTLP_ENDPOINT=ingest.{region}.signoz.cloud:443 go run main.go
```

**NOTE - Default redis address: 127.0.0.1:6379**
...
Refresh the endpoints multiple times to simulate load. Wait for 1–2 minutes, and the telemetry data (traces, metrics, and logs) will appear on your SigNoz dashboard, providing insights into your application's performance.

Below are examples to interact with the e-commerce app:
...
**Explanation:**
- Lists all available products in your e-commerce app.
- **Telemetry**: Each request will generate HTTP request count and duration metrics, along with traces for the `/products` endpoint.

<Figure
  src="/img/blog/2025/11/go-monitoring-products.webp"
  alt="List all products"
  caption="Listed all products"
/>
...
<Figure
  src="/img/blog/2025/11/go-monitoring-new-order.jpeg"
  alt="Create new order"
  caption="Created a new order"
/>
...
**Explanation:**
- Simulates an inventory check with a random delay (100–400ms).
...
The script repeatedly calls the `/products`, `/orders`, `/checkInventory`, `/cpuTest`, and `/concurrencyTest` endpoints, generating traces, metrics, and logs. This simulates real user activity, enabling SigNoz to visualizevisualise request latency, business metrics, and Go runtime behaviour in real time.
...
You can monitor application metrics such as application latency, requests per second, error percentage, and more inh the **`Metrics`** tab of SigNoz. 
