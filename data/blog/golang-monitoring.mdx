...
<KeyPointCallout title="Function Breakdown" defaultCollapsed={false}>
...
Make sure to import the variables in all the paths that need them, such as `telemetry/otel.go`, `main.go`, and more. If you still face any dependency-related errors, try running the following to install all the required dependencies:
```bash
go mod tidy
```
